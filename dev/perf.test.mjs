...
/* 800->900 2026-08-02: no engine change — at 800 the budget sat so close to the
   warm-run x4 that ANY sibling process (review agents, a browser suite) tipped
   it over even on a serial run; 850ms flakes reproduced twice. 900 still fails
   loud on a real regression (those double, they don't add 8%). */
test('cycles: full simulate (dual policy + augment re-sim) under 900ms', async () => {
  ...
  await timed(900, () => simulate(m, {seed: 1, n: 5000}));
