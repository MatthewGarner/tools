...
function status(node, root, policy){
...
function head(model, results, ctx, C, W, verdictParts, presentation = false){
  ...
  if(!authored && rec && st && model.root.kind === 'decision'){
    ...
    if(h){ const share = h.a === rec.label ? h.aShare : 1-h.aShare; msg += ' · beats ' + (h.a === rec.label ? h.b : h.a) + ' in ' + Math.round(share*100) + '% of simulations'; }
...
function card(item, model, results, ctx, C, layout, v, tops){
  ...
  let ty=y+23;
  item.lines.forEach((line,i)=>{ out.push('<text x="'+(x+15)+'" y="'+ty+'" font-size="13" font-weight="650" fill="'+C.ink+'">'+field('label',node,node.label,line,'Edit label: '+node.label,!node.implicit&&i===0)+'</text>'); ty+=17; });
  ty+=3;
  if(node.p!=null){ const text=v.prob(node.p), h=ctx.edit&&hot.has('prob:'+node.srcLine); out.push('<text x="'+(x+15)+'" y="'+ty+'" font-size="10.5" fill="'+C.muted+'">'+field('prob',node,node.pRaw||(node.p==='rest'?'rest':''),text,'Edit probability: '+node.label,true,h)+'</text>'); if(h) out.push('<line x1="'+(x+15)+'" y1="'+(ty+2)+'" x2="'+(x+75)+'" y2="'+(ty+2)+'" stroke="'+C.accent+'" stroke-dasharray="1.5,2"/>'); ty+=14; }
  if(node.value){ const text=v.range(node.value), h=ctx.edit&&hot.has('value:'+node.srcLine); out.push('<text x="'+(x+15)+'" y="'+ty+'" font-size="10.5" fill="'+C.muted+'">'+field('value',node,node.valueRaw||'',text,'Edit payoff: '+node.label,true,h)+'</text>'); if(h) out.push('<line x1="'+(x+15)+'" y1="'+(ty+2)+'" x2="'+(x+105)+'" y2="'+(ty+2)+'" stroke="'+C.accent+'" stroke-dasharray="1.5,2"/>'); ty+=14; }
