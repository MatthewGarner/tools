...
function layoutWide(model, ctx, diff, startY){
  ...
  for(const entry of rows){
    ...
    entry.h = (compact ? 4 : 8) + entry.lines.length * entry.leading + (entry.note ? (compact ? 14 : 17) : 0) + (entry.badge ? 15 : 0) + entry.warnLines.length * 13 + (compact ? 4 : 7); entry.y = y; y += entry.h + (entry.stage === 'outcome' ? (compact ? 12 : 20) : (compact ? 7 : 12));
  }
  return {rows, end:y};
}
...
function wide(model, projection, ctx, diff, c){
  ...
  for(const entry of rows){ const node = entry.node, broken = entry.stage === 'assumption' && node.status === 'broken', attrs = ' data-causal-node="' + node.srcLine + '" data-line="' + node.srcLine + '" data-causal-stage="' + entry.stage + '" data-causal-context="' + esc(trail(entry)) + '"' + (entry.parent ? ' data-causal-parent="' + entry.parent.node.srcLine + '"' : '') + (broken ? ' data-causal-claim="broken"' : ''); out.push('<g' + attrs + '>');
    if(ctx.edit) out.push('<rect' + menu(entry) + ' data-hit="" x="' + entry.x + '" y="' + entry.y + '" width="' + entry.w + '" height="' + Math.max(44, entry.h) + '" fill="transparent" pointer-events="all"/>'); if(entry.stage === 'outcome') out.push('<line x1="' + entry.x + '" y1="' + entry.y + '" x2="' + (entry.x + entry.w) + '" y2="' + entry.y + '" stroke="' + c.ink + '" stroke-width="2"/>');
    ...
    if(entry.note){ if(ctx.edit) out.push('<rect' + stateEdit(entry) + ' data-hit="" x="' + entry.x + '" y="' + (ty - 13) + '" width="132" height="44" fill="transparent"/>'); out.push('<text data-causal-state="' + esc(node.status) + '" pointer-events="none" x="' + entry.x + '" y="' + ty + '" font-size="10" font-weight="700" letter-spacing="1.1" fill="' + (broken ? c.err : c.muted) + '">' + esc(entry.note) + '</text>'); ty += 17; }
...
function narrow(model, projection, ctx, diff, c){
  ...
  for(const entry of rows){ const node = entry.node, x = pad + Math.min(entry.depth, 3) * 10, w = W - x - pad, contextLines = entry.parent ? wrapCausal('PATH · ' + trail(entry), '600 9px sans-serif', w, measure) : [], labelLines = wrapCausal(node.label, '600 15px sans-serif', w, measure), note = state(entry), badge = diffLabel(entry, diff), warn = warning(model, node.srcLine), warningLines = warn ? wrapCausal('SOURCE WARNING · ' + warn, '400 10px sans-serif', w, measure) : [], h = Math.max(44, 14 + contextLines.length * 12 + labelLines.length * 18 + (note ? 16 : 0) + (badge ? 15 : 0) + warningLines.length * 13 + 8), broken = entry.stage === 'assumption' && node.status === 'broken', attrs = ' data-causal-node="' + node.srcLine + '" data-line="' + node.srcLine + '" data-causal-stage="' + entry.stage + '" data-causal-context="' + esc(trail(entry)) + '"' + (entry.parent ? ' data-causal-parent="' + entry.parent.node.srcLine + '"' : '') + (broken ? ' data-causal-claim="broken"' : ''); out.push('<g' + attrs + '>');
    if(ctx.edit) out.push('<rect' + menu(entry) + ' data-hit="" x="' + x + '" y="' + y + '" width="' + w + '" height="' + h + '" fill="transparent" pointer-events="all"/>'); out.push('<line x1="' + (x - 9) + '" y1="' + y + '" x2="' + (x - 9) + '" y2="' + (y + h) + '" stroke="' + (entry.stage === 'outcome' ? c.ink : c.border) + '" stroke-width="' + (entry.stage === 'outcome' ? 2 : 1) + '"/>');
    let ty = y + 10; out.push('<text pointer-events="none" x="' + x + '" y="' + ty + '" font-size="9" font-weight="700" letter-spacing="1.2" fill="' + c.muted + '">' + entry.stage.toUpperCase() + '</text>'); ty += 13; if(contextLines.length) out.push('<g data-causal-breadcrumb="' + esc('PATH · ' + trail(entry)) + '" pointer-events="none">'); for(const line of contextLines){ out.push('<text x="' + x + '" y="' + ty + '" font-size="9" font-weight="600" letter-spacing=".3" fill="' + c.muted + '">' + esc(line) + '</text>'); ty += 12; } if(contextLines.length) out.push('</g>'); labelLines.forEach((line, index) => { out.push('<text' + (ctx.edit && index === 0 ? rename(entry) : ' pointer-events="none"') + ' x="' + x + '" y="' + ty + '" font-size="15" font-weight="600" fill="' + c.ink + '">' + esc(line) + '</text>'); ty += 18; });
    if(note){ if(ctx.edit) out.push('<rect' + stateEdit(entry) + ' data-hit="" x="' + x + '" y="' + (ty - 13) + '" width="132" height="44" fill="transparent"/>'); out.push('<text data-causal-state="' + esc(node.status) + '" pointer-events="none" x="' + x + '" y="' + ty + '" font-size="10" font-weight="700" letter-spacing="1" fill="' + (broken ? c.err : c.muted) + '">' + esc(note) + '</text>'); ty += 16; } if(badge){ out.push('<text data-causal-diff="' + esc(badge.toLowerCase()) + '" pointer-events="none" x="' + x + '" y="' + ty + '" font-size="9" font-weight="700" fill="' + c.muted + '">' + esc(badge) + '</text>'); ty += 15; } for(const line of warningLines){ out.push('<text data-causal-diagnostic-line="' + node.srcLine + '" pointer-events="none" x="' + x + '" y="' + ty + '" font-size="10" fill="' + c.muted + '">' + esc(line) + '</text>'); ty += 13; } out.push('</g>'); y += h + 13;
  }
