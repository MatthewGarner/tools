...
test('every parse warning has its owning phase, exact source line and dedupe key', () => {
  ...
  one(model, 'invalid-style', 1, 'parse', 'line 1: style "banana" is not valid — use "brief", "question", "conditions", "agenda", "tree" or "plans"; style read as "brief"');
  ...
  model = parse('style: agenda');
  assert.equal(model.style, 'agenda');
  ...
  model = parse('style: dependencies');
  assert.equal(model.style, 'dependencies');
  assert.equal(model.warnings.length, 0);
