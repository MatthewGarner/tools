...
export function renderZoneAtlasPlate(model,resolved,ro,ctx={},diff=null){
  ...
  const W=1920,H=1080,fx=100,fy=212,fw=1160,fh=650,mx=1334,mw=470,out=[];
  ...
  out.push('<text x="100" y="'+(172+Math.max(0,title.length-1)*50)+'" font-size="14" font-weight="600" letter-spacing="1.25" fill="'+C.muted+'">'+esc((ro.counts||[]).filter(Boolean).join(' · ').toUpperCase())+'</text>');
  ...
  if(diff){out.push('<text x="'+mx+'" y="'+y+'" font-size="13" font-weight="600" letter-spacing="1.35" fill="'+C.muted+'">'+esc(upper(diff.sinceLine))+'</text>');y+=24;for(const dropped of diff.dropped){out.push('<text x="'+mx+'" y="'+y+'" font-size="13" fill="'+C.muted+'">WAS · '+esc(upper(dropped))+'</text>');y+=17;}y+=8;}
