...
Two origins, one repo: the tools above serve straight off the repository root; the energy tools live under [`energy/`](energy/README.md) and serve on a second domain. Because Vercel serves the filesystem before rewrites, each origin's root trio (`index.html` / `sw.js` / `manifest.webmanifest`) is relocated into a subfolder — `home/` for the tools origin, `energy/` for the energy origin — and served back by host-conditioned rewrites. The whole path map lives once in [`dev/origins.mjs`](dev/origins.mjs). `api/` holds the one backend exception (the gauge relay).
